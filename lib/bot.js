function Bot(nick) {
    ...
    this.isDefineRequest = function (message) {
        return this.addressedToMe(message) && this.DEFINE.exec(this.normalizeMessage(message));
    };
    
    this.parseDefineRequest = function (message) {
        var matchData = this.DEFINE.exec(this.normalizeMessage(message));
        if (matchData) {
            return {
                word: matchData[1].trim()
            };
        } else {
            return false;
        }
